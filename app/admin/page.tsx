...
export default function AdminPage() {
  ...
  useEffect(() => {
    async function initAdmin() {
      ...
      if (!session || session.user.email !== emailBos) {
        // Ganti Alert jadi Toast Error (tapi karena redirect cepat, mungkin tak terlihat, tak apa)
  ...
  const handleEditClick = (produk: any) => {
    ...
    toast("✏️ Mode Edit Aktif", { icon: '📝' }); // Toast Info
  };
  ...
  const handleSimpanProduk = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (idProduk) {
        ...
        toast.success("Produk Berhasil Diupdate! ✅", { id: toastId }); // Update toast jadi sukses
      } else {
      ...
      // Reset form tanpa panggil handleBatalEdit biar ga muncul toast batal
      setIdProduk(null); setNama(''); setHarga(''); setDeskripsi(''); setKategori('Ebook'); setLinkMayar(''); setFileGambar(null); setGambarLama('');
  ...
  const handleHapusProduk = async (id: number) => {
    // Confirm bawaan browser tetap dipakai untuk keamanan, tapi setelah hapus pakai Toast
  ...
  const handleUpdateTampilan = async (e: React.FormEvent) => {
    ...
    try {
        ...
        toast.error("Gagal update tema: " + err.message, { id: toastId }); 
    }
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-10 px-4">
      {/* 2. PASANG KOMPONEN TOASTER DI SINI BIAR MUNCUL */}
                ...
                <div className="lg:col-span-1 bg-white p-6 rounded-3xl shadow-lg h-fit sticky top-4">
                    <div className="flex justify-between items-center mb-4 border-b pb-2">
                        <h2 className="text-xl font-bold text-gray-800">
                            {idProduk ? '✏️ Edit Produk' : '➕ Tambah Produk'}
                        </h2>
                        {idProduk && (
                            <button onClick={handleBatalEdit} className="text-xs text-red-500 font-bold hover:underline">Batal Edit</button>
                        )}
                    </div>
                    ...
                    <form onSubmit={handleSimpanProduk} className="space-y-4">
                        ...
                        <div className="bg-gray-50 p-3 rounded-lg border border-dashed border-gray-300">
                            <span className="text-xs font-bold text-gray-500 block mb-1">Gambar Produk</span>
                            {idProduk && gambarLama && !fileGambar && (
                                <div className="mb-2">
                                    <p className="text-[10px] text-gray-400">Gambar Saat Ini:</p>
                                    <img src={gambarLama} className="h-16 w-16 object-cover rounded shadow-sm" />
                                </div>
                            )}
                            <input type="file" accept="image/*" className="w-full text-sm" onChange={e => setFileGambar(e.target.files?.[0] || null)} />
                        </div>

                        <button disabled={uploading} className={`w-full py-3 rounded-xl font-bold text-white transition shadow-lg ${idProduk ? 'bg-orange-500 hover:bg-orange-600' : 'bg-blue-600 hover:bg-blue-700'}`}>
                            {uploading ? 'Memproses...' : (idProduk ? 'Update Produk 💾' : 'Simpan Produk 🚀')}
                        </button>
                    </form>
                </div>
                        ...
                        {daftarProduk.map(item => (
                            <div key={item.id} className={`flex items-center gap-3 p-3 border rounded-xl transition ${idProduk === item.id ? 'bg-orange-50 border-orange-300 ring-1 ring-orange-200' : 'hover:bg-gray-50'}`}>
                                ...
                                <div className="flex-grow">
                                    <p className="font-bold text-sm text-gray-800">{item.nama_produk}</p>
                                    <div className="flex items-center gap-2 text-xs text-gray-500">
                                        <span className="font-bold text-blue-600">Rp {item.harga?.toLocaleString()}</span>
                                        <span>• {item.kategori}</span>
                                    </div>
                                </div>
                                <div className="flex gap-2">
                                    <button onClick={() => handleEditClick(item)} className="bg-orange-100 text-orange-600 p-2 rounded-lg hover:bg-orange-500 hover:text-white transition" title="Edit Produk">✏️</button>
                                    <button onClick={() => handleHapusProduk(item.id)} className="bg-red-100 text-red-600 p-2 rounded-lg hover:bg-red-600 hover:text-white transition" title="Hapus Produk">🗑️</button>
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            <div className="max-w-2xl mx-auto bg-white p-8 rounded-3xl shadow-xl border border-purple-100">
                ...
                <form onSubmit={handleUpdateTampilan} className="space-y-6">
                    ...
                    <div className="p-4 border-2 border-dashed border-pink-200 rounded-xl bg-pink-50">
                        <label className="block text-sm font-bold text-pink-700 mb-2">Ganti Background Footer</label>
