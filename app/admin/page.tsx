...
export default function AdminPage() {
  ...
  // Default tampil = true kalau admin yang nambah sendiri
  const [tampilTesti, setTampilTesti] = useState(true); 
  ...
  // LOGIKA PRODUK & TAMPILAN (SAMA SEPERTI SEBELUMNYA)
  ...
  const toggleStatusTesti = async (id: number, statusSaatIni: boolean) => {
      const toastId = toast.loading("Mengubah status...");
      await supabase.from('testimoni').update({ tampil: !statusSaatIni }).eq('id', id);
      toast.success(statusSaatIni ? "Testimoni Disembunyikan 🚫" : "Testimoni Ditampilkan ✅", { id: toastId });
      ambilDaftarTesti();
  };
  ...
  const handleHapusTesti = async (id: number) => { if(!confirm("Hapus selamanya?")) return; await supabase.from('testimoni').delete().eq('id', id); toast.success("Dihapus!"); ambilDaftarTesti(); };
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-10 px-4">
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {activeTab === 'testimoni' && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                {/* FORM INPUT TESTIMONI (ADMIN) */}
                <div className="lg:col-span-1 bg-white p-6 rounded-3xl shadow-lg h-fit sticky top-4 border border-green-100">
                    <div className="flex justify-between items-center mb-4 border-b pb-2"><h2 className="text-xl font-bold text-gray-800">{idTesti ? '✏️ Edit Testimoni' : '💬 Tambah Testimoni'}</h2>{idTesti && (<button onClick={handleBatalTesti} className="text-xs text-red-500 font-bold hover:underline">Batal</button>)}</div>
                    <form onSubmit={handleSimpanTesti} className="space-y-4">
                        <div><label className="text-xs text-gray-500 ml-1">Nama Pembeli</label><input type="text" placeholder="Contoh: Budi Santoso" required className="w-full p-2 border rounded-lg" value={namaTesti} onChange={e => setNamaTesti(e.target.value)} /></div>
                        <div><label className="text-xs text-gray-500 ml-1">Role/Pekerjaan</label><input type="text" placeholder="Contoh: YouTuber" required className="w-full p-2 border rounded-lg" value={roleTesti} onChange={e => setRoleTesti(e.target.value)} /></div>
                        <div><label className="text-xs text-gray-500 ml-1">Isi Testimoni</label><textarea placeholder="Tulis komentar positif..." required className="w-full p-2 border rounded-lg h-24" value={textTesti} onChange={e => setTextTesti(e.target.value)}></textarea></div>
                        <div><label className="text-xs text-gray-500 ml-1">Pilih Avatar (Emoji)</label><select className="w-full p-2 border rounded-lg bg-white text-xl" value={avatarTesti} onChange={e => setAvatarTesti(e.target.value)}><option>😎</option><option>👨‍💻</option><option>👩‍💻</option><option>🧕</option><option>🤴</option><option>👸</option><option>📹</option><option>📸</option><option>🛒</option><option>🔥</option></select></div>
                        <button className={`w-full py-3 rounded-xl font-bold text-white transition shadow-lg ${idTesti ? 'bg-orange-500 hover:bg-orange-600' : 'bg-green-600 hover:bg-green-700'}`}>{idTesti ? 'Update Testimoni' : 'Simpan & Tampilkan'}</button>
                    </form>
                </div>

                {/* DAFTAR MODERASI */}
                <div className="lg:col-span-2 bg-white p-6 rounded-3xl shadow-lg border border-green-100">
                    <h2 className="text-xl font-bold mb-4 border-b pb-2">Moderasi Testimoni ({daftarTesti.length})</h2>
                            ...
                            <div key={t.id} className={`p-4 rounded-xl border flex gap-4 items-start ${t.tampil ? 'bg-white border-green-200' : 'bg-red-50 border-red-200'}`}>
                                ...
                                <div className="flex-grow">
                                    <div className="flex justify-between items-start">
                                        <div>
                                            <h3 className="font-bold text-gray-800">{t.nama}</h3>
                                            <p className="text-xs text-gray-500 font-bold uppercase">{t.role}</p>
                                        </div>
                                        {/* 🔥 TOMBOL MODERASI */}
                                        <div className="flex gap-2">
                                            <button onClick={() => toggleStatusTesti(t.id, t.tampil)} className={`text-xs px-3 py-1 rounded font-bold text-white ${t.tampil ? 'bg-green-500 hover:bg-green-600' : 'bg-gray-400 hover:bg-gray-500'}`}>
                                                {t.tampil ? '✅ Tampil' : '🚫 Hidden'}
                                            </button>
                                            <button onClick={() => handleEditTesti(t)} className="text-xs bg-orange-100 text-orange-600 px-2 py-1 rounded hover:bg-orange-200">✏️</button>
                                            <button onClick={() => handleHapusTesti(t.id)} className="text-xs bg-red-100 text-red-600 px-2 py-1 rounded hover:bg-red-200">🗑️</button>
                                        </div>
                                    </div>
                                    <p className="text-sm text-gray-600 mt-2 italic">"{t.text}"</p>
                                    {!t.tampil && <span className="text-[10px] bg-red-200 text-red-800 px-2 py-0.5 rounded mt-2 inline-block font-bold">Menunggu Persetujuan Admin</span>}
                                </div>
                            </div>
                        ))}
                    </div>
